...
class EditBandViewModel {
    ...
    func editGroup(id: Group.ID, name: String, englishName: String, biography: String?, since: Date?, thumbnail: UIImage?, hometown: String?) {
        let imageUrl: String = "aaa"
        
        var uri = EditGroup.URI()
        uri.id = id
        let request = EditGroup.Request(name: name, englishName: englishName, biography: biography, since: since, artworkURL: URL(string: imageUrl), hometown: hometown)
    }
}
