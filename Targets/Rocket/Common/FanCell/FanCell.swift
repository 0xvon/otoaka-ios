...
//    }()
    ...
    func inject(input: Input) {
        ...
        usernameLabel.text = input.user.username.map { "@\($0)" }
        profileLabel.text = [
...
//        biographyTextView.text = input.user.biography
    ...
    func setup() {
        ...
        NSLayoutConstraint.activate([
            ...
            fanArtworkImageView.leftAnchor.constraint(equalTo: leftAnchor),
            fanArtworkImageView.bottomAnchor.constraint(lessThanOrEqualTo: bottomAnchor, constant: -16)
        ])
